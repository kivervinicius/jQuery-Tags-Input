...
(function ($) {
	...
	$.fn.addTag = function (value, options) {
		...
		this.each(function () {
			...
			value = $.fn.tagsInput.testOnlyNumber(value, options, this);
			if(value === "-1"){
				return false;
			}
			var skipTag = false;
			...
			if (settings.interactive) {
				...
				if (settings.autocomplete_url !== undefined) {
					...
					if (jQuery.Autocompleter !== undefined) {
						...
						fakeInput.bind('result', data, function (event, data, formatted) {
							if (data) {
								$('#' + id).addTag(data[0] + "", {
									focus: true,
					...
					fakeInput.bind('blur', data, function (event) {
						...
						if(val !== ''){
							val = $.fn.tagsInput.testOnlyNumber(val, event.data, this);
							if(val === "-1"){
								return false;
							}
							if (val !== '' && val !== d) {
								if ((event.data.minChars <= val.length) && 
									(!event.data.maxChars || (event.data.maxChars >= val.length))){

									$(event.data.real_input).addTag(val, {
										focus: true,
										unique: (settings.unique)
									});
								}
				...
				data.removeWithBackspace && fakeInput.bind('keydown', function (event) {
	...
	$.fn.tagsInput.testOnlyNumber = function (value, options, fake_input) {
		value = jQuery.trim(value);
		if(options.onlyNumber){
			if(isNaN(value)){
				...
				if(options.strZero){
					value = $.fn.tagsInput.strZero(value, options.strZero);
				}
			} 
		}
		return value;
	...
	$.fn.tagsInput.strZero = function strZero(n, len) {
		var sign = '';
		var s = n;
		if(n === ''){ 
			return ""; 
